...
public class MultiUrl {
        ...
        this.mInterceptor = new Interceptor() {
            ...
            public Response intercept(Chain chain) throws IOException {
                ...
                if (request != null && request.url() != null && !request.url().toString().contains(mBaseUrl)) {
                    NaViLogUtil.i(TAG, "无统一BaseUrl不拦截:" + request.url() + ";BaseUrl:" + mBaseUrl);
                    return chain.proceed(request);
                }
    ...
    public Request processRequest(Request request) {
        ...
        if (null != httpUrl) {
            ...
            NaViLogUtil.i(MultiUrl.TAG, "Header 模式重定向Url:BaseRefreshFragment Url is { " + mBaseUrl + " }" + ";New Url is { " + newUrl + " }" + ";Old Url is { " + request.url() + " }");
            return newBuilder
                    .url(newUrl)
                    .build();
        }
        ...
        if (null != httpUrl) {
            NaViLogUtil.i(MultiUrl.TAG, "Method 模式重定向Url:BaseRefreshFragment Url is { " + mBaseUrl + " }" + ";New Url is { " + httpUrl + " }" + ";Old Url is { " + request.url() + " }");
            return newBuilder
                    .url(httpUrl)
                    .build();
        }
        ...
        if (httpUrlBase != null && !httpUrlBase.toString().equals(mBaseUrl)) {
            ...
            NaViLogUtil.i(MultiUrl.TAG, "重定向Url:BaseRefreshFragment Url is { " + httpUrlBase.toString() + " }" + ";New Url is { " + httpNew + " }" + ";Old Url is { " + request.url() + " }");
            return newBuilder
                    .url(httpNew)
                    .build();
        }
    ...
    private HttpUrl getMethodHttpUrl(Request request) {
        ...
        NaViLogUtil.d(TAG, "BaseRefreshFragment Url is { " + mBaseUrl + " }" + ";Old Url is{" + url.newBuilder().toString() + "};Method is <<" + methodKey + ">>");
        //如果
    ...
    private String getHeaderBaseUrlKey(Request request) {
        ...
        if (heads != null) {
            NaViLogUtil.i(TAG, "header:" + heads.toString());
        }
