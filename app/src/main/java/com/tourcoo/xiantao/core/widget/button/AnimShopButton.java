package com.tourcoo.xiantao.core.widget.button;
...
public class AnimShopButton extends View {
    ...
    protected void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < indexCount; i++) {
            ...
            } else if (index == R.styleable.AnimShopButton_numberTextColor) {
                mTextColor = ta.getColor(index, ContextCompat.getColor(context, R.color.black));
            }
        }
        ta.recycle();
        ...
        mAniDel.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                if (mCount == 0) {
                    Log.d("AnimShopButton", "现在还是0onAnimationEnd() called with: animation = [" + animation + "]");
                    if (mAnimExpandHint != null && !mAnimExpandHint.isRunning()) {
    ...
    private void initDefaultValue(Context context) {
        mGapBetweenCircle = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 28, context.getResources().getDisplayMetrics());
...
/*        mMaxCount = 4;
    ...
    protected void onDelClick() {
        if (mCount > minCount) {
            ...
            if (null != mOnAddDelListener) {
                mOnAddDelListener.onDelFailed(mCount, IOnAddDelListener.FailType.COUNT_MIN);
            }
        }
