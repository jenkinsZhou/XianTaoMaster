...
//    private int cursor; // 光标
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        // a. 点击状态的左侧图标
        // 1. 获取资源ID
        ic_left_clickResID = typedArray.getResourceId(R.styleable.SuperEditText_ic_left_click, R.mipmap.ic_mobile_phone);
        // 2. 根据资源ID获取图标资源（转化成Drawable对象）
        ic_left_click = getResources().getDrawable(ic_left_clickResID);
        lineEnable = typedArray.getBoolean(R.styleable.SuperEditText_lineEnable, true);
        // 3. 设置图标大小
        ...
        left_width = typedArray.getInteger(R.styleable.SuperEditText_left_width, dp2px(15));
        left_height = typedArray.getInteger(R.styleable.SuperEditText_left_height, dp2px(19));
        ic_left_click.setBounds(left_x, left_y, left_width + left_x, left_height + left_y);
        // Drawable.setBounds(x,y,width,height) = 设置Drawable的初始位置、宽和高等信息
        // x = 组件在容器X轴上的起点、y = 组件在容器Y轴上的起点、width=组件的长度、height = 组件的高度
        // b. 未点击状态的左侧图标
        // 1. 获取资源ID
        ic_left_unclickResID = typedArray.getResourceId(R.styleable.SuperEditText_ic_left_unclick, R.mipmap.ic_mobile_phone);
        // 2. 根据资源ID获取图标资源（转化成Drawable对象）
        ...
        // 1. 获取资源ID
        ic_deleteResID = typedArray.getResourceId(R.styleable.SuperEditText_ic_delete, R.mipmap.ic_clean_up);
        // 2. 根据资源ID获取图标资源（转化成Drawable对象）
