package com.tourcoo.xiantao.widget.sku.widget;
...
public class FlowLayout extends ViewGroup {
    ...
    public FlowLayout(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(
                attrs, R.styleable.LabelLayout, 0, 0);
        try {
            mFlow = a.getBoolean(R.styleable.LabelLayout_flow, DEFAULT_FLOW);
            try {
                mChildSpacing = a.getInt(R.styleable.LabelLayout_childSpacing, DEFAULT_CHILD_SPACING);
            } catch (NumberFormatException e) {
                mChildSpacing = a.getDimensionPixelSize(R.styleable.LabelLayout_childSpacing, (int) dpToPx(DEFAULT_CHILD_SPACING));
            }
            try {
                mChildSpacingForLastRow = a.getInt(R.styleable.LabelLayout_childSpacingForLastRow, SPACING_UNDEFINED);
            } catch (NumberFormatException e) {
                mChildSpacingForLastRow = a.getDimensionPixelSize(R.styleable.LabelLayout_childSpacingForLastRow, (int) dpToPx(DEFAULT_CHILD_SPACING));
            }
            try {
                mRowSpacing = a.getInt(R.styleable.LabelLayout_rowSpacing, 0);
            } catch (NumberFormatException e) {
                mRowSpacing = a.getDimension(R.styleable.LabelLayout_rowSpacing, dpToPx(DEFAULT_ROW_SPACING));
            }
            mMaxRows = a.getInt(R.styleable.LabelLayout_maxRows, DEFAULT_MAX_ROWS);
            mRtl = a.getBoolean(R.styleable.LabelLayout_rtl, DEFAULT_RTL);
        } finally {
            a.recycle();
        }
    }
