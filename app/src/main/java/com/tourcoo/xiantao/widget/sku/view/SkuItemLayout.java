...
import com.tourcoo.xiantao.widget.sku.ScreenUtils;
...
public class SkuItemLayout extends LinearLayout {
    ...
    private void init(Context context) {
        ...
        attributeNameTv.setTextColor(context.getResources().getColor(TourCooUtil.getColor(R.color.greenCommon)));
    ...
    public void optionItemViewSelectStatus(SkuAttribute selectValue) {
        for (int i = 0; i < attributeValueLayout.getChildCount(); i++) {
            SkuItemView itemView = (SkuItemView) attributeValueLayout.getChildAt(i);
            if (selectValue.getSpec_value().equals(itemView.getAttributeValue())) {
                itemView.setEnabled(true);
    ...
    private void onSkuItemClicked(int position, SkuItemView view) {
        ...
        attribute.setItem_id(attributeNameTv.getText().toString());
        attribute.setSpec_value(view.getAttributeValue());
