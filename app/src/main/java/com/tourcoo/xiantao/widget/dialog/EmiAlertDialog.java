 ...
 */
public class EmiAlertDialog extends BaseDialog<EmiAlertDialog> {


    public EmiAlertDialog(Context context) {
    ...
    protected static class DividerBuilder<T extends DividerBuilder> extends Builder<T> {
        ...
        protected Drawable getButtonDrawable(int gravity) {
            ...
            boolean flag = mBackgroundRadius > 0
            ...
            if (flag) {
                ...
                float[] radiusArr = new float[8];
                //左边第一个/或只有一个
                if (gravity == Gravity.LEFT || gravity == Gravity.BOTTOM) {
                    radiusArr[6] = mBackgroundRadius;
                    radiusArr[7] = mBackgroundRadius;
                }
                //右边/或只有一个
                if (gravity == Gravity.RIGHT || gravity == Gravity.BOTTOM) {
        ...
        protected void addButton(CharSequence sequence, float textSize,
                                 ColorStateList textColor, boolean fakeBoldText, final int witch, final DialogInterface.OnClickListener listener) {
    ...
    private static abstract class Builder<T extends Builder> extends BasisBuilder<T> implements ICreateButtons {
        ...
        public EmiAlertDialog create() {
            int margin = dp2px(16);
            View contentView = createContentView();
            mDialog = new EmiAlertDialog(mContext);
            mDialog.setContentView(contentView);
            setDialog();
            mDialog.setGravity(Gravity.CENTER);
            mDialog.setMargin(margin, margin, margin, margin);
            afterSetContentView();
            return (EmiAlertDialog) mDialog;
        }
