...
/**
 * @author :JenkinsZhou
 * @description :金钱计算工具类(由于浮点数值采用二进制系统表示，而在二进制系统中无法精确的表示分数
 * 需要在数值计算中不含有任何舍入误差，应该使用BigDecimal类)
 * @company :途酷科技
 * @date 2019年05月17日15:23
...
public class DecimalUtil {
     ...
     * @param scale 小数点保留位数
     */
    public static String multiplyWithScale(double v1, double v2, int scale) {
        BigDecimal b1 = new BigDecimal(v1);
        BigDecimal b2 = new BigDecimal(v2);
        BigDecimal result = b1.multiply(b2);
        result = result.setScale(scale);
