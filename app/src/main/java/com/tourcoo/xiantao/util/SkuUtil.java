...
public class SkuUtil {
    ...
    private static void add2SKUResult(HashMap<String, BaseSkuModel> result, ArrayList<String> newKeyList, BaseSkuModel skuModel) {
        ...
        if (result.keySet().contains(key)) {
            result.get(key).setStock_num(result.get(key).getStock_num() + skuModel.getStock_num());
            result.get(key).setGoods_price(skuModel.getGoods_price());
        } else {
            result.put(key, new BaseSkuModel(skuModel.getGoods_price(), skuModel.getStock_num()));
        }
    }
